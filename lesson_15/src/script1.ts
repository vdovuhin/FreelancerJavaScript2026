...
class Firm {
  ...
  getYearsOfExistence(): number {
    const currentYear = new Date().getFullYear();
    return currentYear - this.foundationDate.year;
  }
  getBranchesInCity(city: string): Branches[] {
    return this.branches.filter(
      (branch) => branch.city.toLowerCase() === city.toLowerCase(),
    );
  }
  getServicesByCostAndDuration(cost: number, duration: number): Services[] {
...
const firm = new Firm(
  "Tech Solutions",
  new FondDate(2010, 5), // Передаємо об'єкт класу FoundationDate
  [
