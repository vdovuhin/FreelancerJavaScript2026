...
function getPlayField(rowNumber, colNumber) {
    return Array.from({ length: rowNumber }, () => new Array(colNumber).fill(0));
}
// 2. Функція розстановки кораблів
function getGameField(rowNumber, colNumber, shipNumber) {
...
function gameNavaBattle(rowNumber, colNumber, shipNumber, missleNumber) {
    ...
    while (leftShipsNumber > 0 && leftMissileNumber > 0) {
        ...
        if (userRowInput === null || userColInput === null)
            break;
    ...
    if (leftShipsNumber === 0)
        alert('Перемога! Всі кораблі знищено!');
    else
        alert('Гра закінчена. Снаряди вичерпано.');
}
